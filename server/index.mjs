...
app.get("/api/users", async(req, res) => {
  ...
  if(error){
    res.status(404).json({status: "error", message: `找不到使用者 ${error.message}`});
    return false;
  }
...
app.post("/api/users", upload.none(), async (req, res) => {
  ...
  if(error){
    res.status(400).json({status: "error", message: error.message ? error.message : "資料輸入錯誤"});
    return false;
  }
...
app.get("/api/users/search", async (req, res) => {
  ...
  res.status(200).json({status: "success", message: "使用者搜尋成功", user: others });
});
